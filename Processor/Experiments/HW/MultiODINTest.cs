...
public class MultiODINTest : Experiment
{
    ...
    private Controller1 AddController(SNN snn, int x, int y)
    {
        var controllerCoord = new MeshCoord(x, y);
        var controller = sim.AddActor(new Controller1(controllerCoord, snn, 0, 100_000, name: "controller"));
    ...
    private (Core1, NI1) AddCore(Controller1 controller, ODINDelayModel delayModel, int size, int x, int y, string name)
    {
        var coreCoord = new MeshCoord(x, y);
        var core = sim.AddActor(new Core1(coreCoord, size, delayModel, enableRefractory: true, name: name));
        ...
        var coreNi = sim.AddActor(new NI1(coreCoord, (MeshCoord)controller.GetLocation()));
